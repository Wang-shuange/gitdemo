...
extern uint8  	use_II2C;																/*是否使用II2C中的IP配置信息*/
/*定义SPI1作为W5500的硬件接口*/
...
#define WIZ_INT								GPIO_Pin_14								/*定义W5500的INT管脚*/
#endif
...
#define II2C_MSG_LEN        	sizeof(II2C_MSG)

#define IP_FROM_DEFINE	           	 			0                /*使用初始定义的IP信息*/
#define IP_FROM_DHCP	              			1                /*使用DHCP获取IP信息*/
#define IP_FROM_II2C	                    2								 /*使用II2C定义的IP信息*/
...
#pragma pack()
#pragma pack(1)
/*此结构体定义了II2C写入的几个变量，可按需修改*/
typedef struct _II2C_MSG	                    
{
	uint8 mac[6];																							/*MAC地址*/
  uint8 lip[4];																							/*local IP本地IP地址*/
  uint8 sub[4];																							/*子网掩码*/
...
void write_config_to_II2C(void);													/*写配置信息到II2C中*/
void read_config_from_II2C(void);													/*从II2C中读出信息*/
