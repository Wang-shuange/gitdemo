...
uint8_t Server_IP_Buffer[BufferSize]="10.199.61.250";
uint8_t Server_Port_Buffer[BufferSize]="8443";
...
//    SPI_FLASH_BufferWrite((uint8_t*)code_password, 0x00020, Password_BufferSize);
//    SPI_FLASH_BufferWrite((uint8_t*)Read_IP, 0x00040, Read_IP_BufferSize);	
//    printf("Read_IP: %s\r\n",Read_IP);		
//		/* 将刚刚写入的数据读出来放到接收缓冲区中 */
//		SPI_FLASH_BufferRead(SSID_Buffer, 0x00000, IP_Port_BufferSize);
//		printf("读出的数据为    SSID：%s    ", SSID_Buffer);
//		printf("Lenth:%d\r\n",strlen((char*)SSID_Buffer));
//		SPI_FLASH_BufferRead(Password_Buffer, 0x00020, Password_BufferSize);
//		printf("读出的数据为  Passrd：%s    ", Password_Buffer);
//    printf("Lenth:%d\r\n",strlen((char*)Password_Buffer));	
//    SPI_FLASH_BufferRead(Read_IP_Buffer, 0x00040, Read_IP_BufferSize);
//		printf("读出的数据为      IP：%s    ", Read_IP_Buffer);
//    printf("Lenth:%d\r\n",strlen((char*)Read_IP_Buffer));	
//		memset(code_password,0,30);
//		memset(code_ssid,0,30);
	}
}
void Flash_Read_AP_Pass(void)
{
		SPI_FLASH_BufferRead(SSID_Buffer, 0x00000, IP_Port_BufferSize);
...
{
//	SPI_FLASH_SectorErase(0x01F90);	
//	SPI_FLASH_SectorErase(0x01FA0);	
	SPI_FLASH_SectorErase(0x02000);	 
	/* 将发送缓冲区的数据写到flash中 */
	// 这里写一页，一页的大小为256个字节
	SPI_FLASH_BufferWrite(APP_FLAGx, 0x02000, APP_FLAGx_BufferSize);	
	/* 将刚刚写入的数据读出来放到接收缓冲区中 */
	SPI_FLASH_BufferRead(APP_FLAGx_Buffer, 0x02000, APP_FLAGx_BufferSize);
	printf("读出的数据为：%s\r\n", APP_FLAG_Buffer);
//	printf("Lenth:%d\r\n",strlen((char*)APP_FLAG_Buffer));
//	SPI_FLASH_BufferRead(Password_Buffer, 0x01FB0, Password_BufferSize);
//	printf("读出的数据为：%s ", Password_Buffer);
//	printf("Lenth:%d\r\n",strlen((char*)Password_Buffer));	
}
char Flash_Read(void)
{
	...
	printf("保存的FLAG数据：%s\r\n", APP_FLAG_Buffer);
	printf("十进制接收数据: ");
	for(i=0;i<9;i++)printf("%d", APP_FLAG_Buffer[i]);
	printf("\r\n");
	if(APP_FLAG_Buffer[1]!=255&&APP_FLAG_Buffer[1]!=48&&APP_FLAG_Buffer[3]!=255&&APP_FLAG_Buffer[3]!=48)str=1;
...
//	SPI_FLASH_BufferWrite(APP_FLAG, 0x02000, APP_FLAG_BufferSize);
//	SPI_FLASH_BufferWrite(Password, 0x01FB0, Password_BufferSize);		
//	/* 将刚刚写入的数据读出来放到接收缓冲区中 */
//	SPI_FLASH_BufferRead(APP_FLAG_Buffer, 0x01000, APP_FLAG_BufferSize);
//	printf("读出的数据为：%s\r\n", APP_FLAG_Buffer);
	SPI_FLASH_BufferRead(APP_FLAG_Buffer, 0x01000, APP_FLAG_BufferSize);
...
void Flash_Write_MAC(void)
{
	sprintf((char*)MAC_ID,"%s",MAC);
	printf("MAC_ID: %s\r\n",MAC_ID);
	SPI_FLASH_SectorErase(0x05000);	
	SPI_FLASH_BufferWrite(MAC_ID, 0x05000, MAC_ID_BufferSize);
