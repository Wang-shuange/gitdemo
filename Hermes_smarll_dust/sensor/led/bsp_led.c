...
{		
		...
		/*开启LED相关的GPIO外设时钟*/
		RCC_APB2PeriphClockCmd( LED1_GPIO_CLK| LED5_GPIO_CLK|LED2_GPIO_CLK| LED7_GPIO_CLK|LED13_GPIO_CLK|RCC_APB2Periph_GPIOB|RCC_APB2Periph_GPIOC|RCC_APB2Periph_GPIOD  , ENABLE);
		/*选择要控制的GPIO引脚*/
...
//RGB2与RGB3初始化	
		GPIO_InitStructure.GPIO_Pin = RGB1_GPIO_PIN|RGB2_GPIO_PIN|RGB3_GPIO_PIN;
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;    
		GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; 
		GPIO_Init(GPIOC, &GPIO_InitStructure);	
		GPIO_InitStructure.GPIO_Pin = RGB31_GPIO_PIN|RGB32_GPIO_PIN|RGB33_GPIO_PIN;
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;    
		GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; 
    ...
    GPIO_InitStructure.GPIO_Pin = LED7_GPIO_PIN|LEDx_GPIO_PIN;     //LED7 Wifi使能引腳(A0) //LEDx   WIFI RST (A8)
		...
		GPIO_InitStructure.GPIO_Pin = LED16_GPIO_PIN|LED18_GPIO_PIN;//Motion
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;    
		GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; 
		GPIO_Init(LED13_GPIO_PORT, &GPIO_InitStructure);
		
		GPIO_InitStructure.GPIO_Pin =GPIO_Pin_0;//煙感
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;    
		GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; 
...
//RGB2		
		GPIO_SetBits(RGB2_GPIO_PORT , RGB1_GPIO_PIN);
		GPIO_SetBits(RGB2_GPIO_PORT , RGB2_GPIO_PIN);	
    GPIO_SetBits(RGB2_GPIO_PORT , RGB3_GPIO_PIN);
//RGB3
		GPIO_SetBits(RGB31_GPIO_PORT, RGB31_GPIO_PIN);
		GPIO_SetBits(RGB31_GPIO_PORT, RGB32_GPIO_PIN);	
    GPIO_SetBits(RGB31_GPIO_PORT, RGB33_GPIO_PIN);
...
//	
		GPIO_ResetBits(RGB1_GPIO_PORT , RGB1_GPIO_PIN);//RGB2
		GPIO_ResetBits(RGB2_GPIO_PORT , RGB2_GPIO_PIN);	
    GPIO_ResetBits(RGB3_GPIO_PORT , RGB3_GPIO_PIN);
	
		GPIO_ResetBits(RGB31_GPIO_PORT, RGB31_GPIO_PIN);//RGB3
		GPIO_ResetBits(RGB32_GPIO_PORT, RGB32_GPIO_PIN);	
    GPIO_ResetBits(RGB33_GPIO_PORT, RGB33_GPIO_PIN);

	  delay_s(3);
	  GPIO_ResetBits(LED1_GPIO_PORT, LED1_GPIO_PIN);//RGB1
		GPIO_SetBits(LED2_GPIO_PORT, LED2_GPIO_PIN);	
    GPIO_SetBits(LED3_GPIO_PORT, LED3_GPIO_PIN);
	
		GPIO_ResetBits(RGB2_GPIO_PORT , RGB1_GPIO_PIN);//RGB2
		GPIO_SetBits(RGB2_GPIO_PORT , RGB2_GPIO_PIN);	
    GPIO_SetBits(RGB2_GPIO_PORT , RGB3_GPIO_PIN);
//	
		GPIO_ResetBits(RGB32_GPIO_PORT , RGB31_GPIO_PIN);//RGB3
		GPIO_SetBits(RGB32_GPIO_PORT , RGB32_GPIO_PIN);	
    GPIO_SetBits(RGB32_GPIO_PORT , RGB33_GPIO_PIN);
}
void smartconfig_LED(void)
{
	  ...
	  GPIO_SetBits(LED2_GPIO_PORT, LED2_GPIO_PIN);//绿	
		...
		GPIO_SetBits(RGB2_GPIO_PORT , RGB1_GPIO_PIN);//RGB2
		GPIO_SetBits(RGB2_GPIO_PORT , RGB2_GPIO_PIN);	
	
		GPIO_SetBits(RGB32_GPIO_PORT , RGB31_GPIO_PIN);//RGB3
		GPIO_SetBits(RGB32_GPIO_PORT , RGB32_GPIO_PIN);	
	  for(i=0;i<5;i++)
	{
    GPIO_ResetBits(LED3_GPIO_PORT, LED3_GPIO_PIN);//红
		GPIO_ResetBits(RGB2_GPIO_PORT , RGB3_GPIO_PIN);
    GPIO_ResetBits(RGB32_GPIO_PORT , RGB33_GPIO_PIN);
	  delay_ms(400);
...
//	
    ...
    GPIO_SetBits(LED3_GPIO_PORT, LED3_GPIO_PIN);//红
//	
		GPIO_SetBits(RGB1_GPIO_PORT , RGB1_GPIO_PIN);//RGB2
		GPIO_ResetBits(RGB2_GPIO_PORT , RGB2_GPIO_PIN);	
    GPIO_SetBits(RGB3_GPIO_PORT , RGB3_GPIO_PIN);
	
		GPIO_SetBits(RGB31_GPIO_PORT, RGB31_GPIO_PIN);//RGB3
		GPIO_ResetBits(RGB32_GPIO_PORT, RGB32_GPIO_PIN);	
    GPIO_SetBits(RGB33_GPIO_PORT, RGB33_GPIO_PIN);
...
void Green_Red_LED_ON(void)//橙黄
{
   ...
   GPIO_ResetBits(LED3_GPIO_PORT, LED3_GPIO_PIN);//红
	 ...
	 GPIO_ResetBits(RGB2_GPIO_PORT , RGB2_GPIO_PIN);//2	
   GPIO_ResetBits(RGB2_GPIO_PORT , RGB3_GPIO_PIN);
	
   GPIO_ResetBits(RGB32_GPIO_PORT , RGB32_GPIO_PIN);//3
   GPIO_ResetBits(RGB32_GPIO_PORT , RGB33_GPIO_PIN);
}
void Green_Red_LED_OFF(void)//橙黄
{
   ...
   GPIO_SetBits(LED3_GPIO_PORT, LED3_GPIO_PIN);//红
	 ...
	 GPIO_SetBits(RGB2_GPIO_PORT , RGB2_GPIO_PIN);//2	
   GPIO_SetBits(RGB2_GPIO_PORT , RGB3_GPIO_PIN);
	
   GPIO_SetBits(RGB32_GPIO_PORT , RGB32_GPIO_PIN);//3
   GPIO_SetBits(RGB32_GPIO_PORT , RGB33_GPIO_PIN);
