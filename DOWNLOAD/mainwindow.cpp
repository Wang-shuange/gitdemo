...
void MainWindow::killFlashloader()
{
    DWORD dwEC = 0;
    BOOL b = GetExitCodeProcess(
                hP,     // handle to the process
                &dwEC              // termination status
                );
    if(b)
        TerminateProcess( hP, dwEC );
    WaitForSingleObject(hP, INFINITE);//等待打开的进程结束并关闭相关句柄
    ::CloseHandle(hT);
    ::CloseHandle(hP);
}
...
void MainWindow::handleTimeout()
{
    ...
    if(myText.indexOf("Unrecognized device... Please, reset your device then try again")!=-1)
    {
        killFlashloader();
         qDebug()<<"count_ko++:"<<count_ko;
            ui->textBrowser->append(tr("请检查跳帽设置是否正确！\r\n"));
            return;

    }
    if(myText.indexOf("No response from the target, the Boot loader can not be started.")!=-1)
    {
        killFlashloader();
        qDebug()<<"count_ko++:"<<count_ko;
            ui->textBrowser->append(tr("请重启单片机！\r\n"));
      return;
    }
...
{
    ui->textBrowser->append(tr("Hermes Firmware download software"));
    ui->textBrowser->append(tr("latest version：1.0.0.5"));
    ui->textBrowser->append(tr("author：王拴阁"));
    ui->textBrowser->append(tr("upgrade：2018-10-11"));
    ui->textBrowser->append(tr("------change history 2018-10-11-------"));
    ui->textBrowser->append(tr("1.修复无法download bug；"));
    ui->textBrowser->append(tr("2.修复不按按钮程序一直运行问题"));
    ui->textBrowser->append(tr("3.增加不插跳帽和不进行复位的防呆功能"));
    ui->textBrowser->append(tr("4.修复程式中途退出后无法关闭STMFlashLoader bug"));
