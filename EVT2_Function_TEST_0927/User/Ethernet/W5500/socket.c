...
uint16 send(SOCKET s, const uint8 * buf, uint16 len)
{
  ...
  send_data_processing(s, (uint8 *)buf, ret);						 /*Copy data*/
...
uint16 macraw_send( const uint8 * buf, uint16 len )
{
   ...
   IINCHIP_WRITE(Sn_CR(sock_num),Sn_CR_SEND);									/*W5500 send command*/
...
uint16 macraw_recv( uint8 * buf, uint16 len )
{
   ...
   if ( len > 0 )
   {
      ...
      data_len = IINCHIP_READ_RXBUF(0, ptr);						/*Read_data(s, (uint8 *)ptr, data, len) & read data*/
      ptr++;
