...
/*************************************************/
void SSID_Password_Read()
{
		RTL_WIFI_Reset();	
		while(wifi_send_commandx( "+++", "OK", 2500,3 )){}
		delay_ms(1000);delay_ms(1000);delay_ms(1000);
		memset(user_input,0,1024);
/***************************************************************/	
		wifi_send_commandxx ( "AT#StaID\r\n", "OK",3000, 1 );
		printf("SSID: %s\r\n",zhon_zhuan);
		sprintf((char*)SSID,"%s%s","AT#StaID ",zhon_zhuan);
		printf("--->>string: %s ",SSID);
		printf("Lenth:%d\r\n",strlen((char*)SSID));
		memset(zhon_zhuan,0,20);
		wifi_send_commandxx ( "AT#StaSec\r\n", "OK", 3000, 1 );
		printf("Password: %s\r\n",zhon_zhuan);
		sprintf((char*)Password,"%s%s","AT#StaSec ",zhon_zhuan);
		printf("--->>string: %s ",(char*)Password);
		printf("Lenth:%d\r\n",strlen((char*)Password));
		while(wifi_send_commandx( "AT#NWK\r\n", "OK", 3000, 3 ));
		Mac_IP_DisPlay();  //打印Mac与IP地址	
		while(wifi_send_commandx( "AT#Exit\r\n", "OK", 5000, 3 )){}			
		memset(zhon_zhuan,0,20);
		IWDG_Feed();
}
...
//		printf("\r\n 检测到串行flash W25Q64 !\r\n");		
		...
		/* 将发送缓冲区的数据写到flash中 */
		// 这里写一页，一页的大小为256个字节
		SPI_FLASH_BufferWrite(SSID, 0x00000, IP_Port_BufferSize);
    SPI_FLASH_BufferWrite(Password, 0x00020, Password_BufferSize);
